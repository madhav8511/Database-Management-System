...
//Modify the following insert statement with YOUR details... use your roll number for SSN
  static final String INSERT_EMP = "INSERT INTO employee values('Madhav',null,'Girdhar','123789456','2000/01/01','address','M',100000,'333445555',5)";
   ...
   try{
      ...
      if(stmt.executeUpdate(INSERT_EMP)<0) conn.rollback();  //Insert statement... and call commit
      else 
      {
         System.out.println("Enter ssn : ");
         int id = sc.nextInt();
         String query = "update employee set super_ssn = "+id+" where ssn = 123789456";
         if(stmt.executeUpdate(query)<0) conn.rollback();
         else conn.commit();
      }
